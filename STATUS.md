...
- **since:** 2026-07-29T15:21:21-0700
- **task:** commit morning brief 1a+1b+2
<!-- END active-session -->

---

**Last updated:** 2026-07-29 (morning brief Phase 1a, 1b, and 2 complete in the uncommitted tree, prompt v14; Alex's install migrated off Supabase to local mode on the MacBook, single machine, Mini fully retired; build wave COMPLETE; ONE click left: refresh + flip amart-builder/cove public)
...
- Reminders read the SQLite file directly (`scripts/cove-reminders.mjs` opens `data/forge.db` with better-sqlite3), and owner chips spawn `claude` as a child process of the web server (`src/lib/task-sessions/manager.ts`). Both fire on whichever machine runs Cove. Hosting on the Mini put Alex's notifications and Claude sessions on a machine in a cabinet that he never touches.
...
- **Rollback.** Set `NEXT_PUBLIC_FORGE_RUNTIME=supabase` in `.env.local` (a pre-migration copy is saved next to it) and rebuild. Supabase still holds every row as of 2026-07-29 10:26 PT, and `data/backups/migration-2026-07-29/` holds both pre-migration database files plus the full JSON exports.
