...
- Focus sessions (2026-09-03): "Start my day" only commits the plan; the local auto-launch of headless task sessions and the "Claude will start N focus tasks" footer are gone (`useDayRitual.ts`, `MorningArrival.tsx`). Planning / Auto stay headless `claude -p` runs; the plan budget cap is 5.00 (1.50 killed real runs on `error_max_budget_usd` after 4 to 5 minutes; two of Alex's runs died that way), auto stays 3.00. When a run ends the manager posts one clickable Mac notification ("Planning finished: …" / "Planning stopped: … Budget reached before it finished.") whose click opens `claude://resume?session=<id>` in the Claude desktop app (verified: the app lists the session as "Cove: <task>"). Never open the app mid-run: two writers on one transcript. Pills: running is a plain span; finished or stopped is "… · Open in Claude".
- Pipeline page restyled to the Morning Arrival language (2026-09-03): token surfaces only, Arrival shell, eyebrows with count tags, stage trays holding 14px-radius deal cards, dark numbered attention cards (the one literal gradient, copied from ArrivalPlanGrid), press-scale and hover lift, `day-ritual-swap-in` on the detail panel. Only `src/components/crm/PipelineView.tsx` and `CrmSubNav.tsx` changed; globals.css untouched. Live-verified computed styles, reduced motion, dark mode, and stage move / log touch / search by a fresh-context reviewer.
...
- Documentation source commit: `e917859`; private release record: `d155153`. The sanitized public mirror is `PUBLIC` at `46536fa7e21fbbce778c4898b11c569d0e686646` and carries private source SHA `d1551532a5876c6925d62ad157fa89692f6c5ca7` in its manifest.
- A brand-new HTTPS clone of the published repository verified all 421 manifest hashes and the presence of `CODEBASE_GUIDE.md`. Under Node 24.14.0 it had zero npm vulnerabilities and passed TypeScript, zero-warning ESLint, all 958 tests, and the production build. The six known non-fatal Turbopack filesystem-tracing warnings remain.
...
- Private source release: `7f5f9fff32f361feb8c1f1154cb432994bd73943`. Sanitized public mirror: `d9fef78d0c347290475cc064b08eb7fb4bac4c8f`, visibility verified `PUBLIC`.
