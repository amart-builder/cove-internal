...
4. Is it urgent? surface: `now` (text + notification) | `scheduled` (`surface_at` time) | `board` (due date + morning brief is enough). `NOW` is given in UTC and `TIMEZONE` names the operator's zone, so resolve every time you emit against `TIMEZONE` and carry the offset it is in on that date. A `scheduled` reminder reaches the operator's phone, and Cove holds automatic notifications to 08:00-20:00 operator time, so pick an hour inside that window; one outside it waits until the window opens rather than ringing at night.
