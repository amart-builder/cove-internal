...
import {
  ...
  parseArrivalConfig,
} from "./lib/cove-arrival-schedule.mjs";
...
async function loadConfig() {
  const override = coveEnv("ARRIVAL_CONFIG")?.trim();
...
async function fetchReadModel(url) {
  ...
  try {
    ...
    if (!response.ok) throw new Error("Cove did not return a successful day plan response.");
