 ...
 *
 * Matcher settings live in data/forge-meetings.json:
 * {
 *   "enabled": true,
...
const defaultDataDir = process.env.FORGE_DATA_DIR?.trim() ||
  path.join(repoDir, "data");
const DEFAULT_CONFIG_PATH = path.join(defaultDataDir, "forge-meetings.json");
const DEFAULT_EMAIL_CONFIG_PATH = path.join(defaultDataDir, "forge-email.json");
...
export function loadMeetingConfig(file = DEFAULT_CONFIG_PATH) {
  ...
  if (
    ...
    throw new Error("forge-meetings.json is missing enabled, query, window, or processed_label.");
  }
  return {
...
function loadEmailConfig(file = DEFAULT_EMAIL_CONFIG_PATH) {
  ...
  if (typeof parsed?.account_email !== "string" || !parsed.account_email.trim()) {
    throw new Error("forge-email.json is missing account_email.");
  }
  return {
