...
const DEFAULT_DATA_DIR = process.env.FORGE_DATA_DIR || path.join(repoDir, "data");
...
function atlasRoot() {
  if (process.env.FORGE_ATLAS_ROOT?.trim()) {
    return path.resolve(process.env.FORGE_ATLAS_ROOT.trim());
  }
...
function supabaseConfig(env = process.env) {
  ...
  const prefix = env.FORGE_TABLE_PREFIX ?? env.NEXT_PUBLIC_FORGE_TABLE_PREFIX ?? "";
...
function progressPrompt(input) {
  return [
    "Map factual work evidence to open Forge tasks.",
  ...
  return new Promise((resolve, reject) => {
    const child = spawnImpl(
      ...
      [
        ...
        "--strict-mcp-config",
        "--mcp-config",
        path.join(workingDir, "scripts", "forge-empty-mcp.json"),
        "--model",
        "claude-opus-5",
        "--output-format",
        "json",
  ...
  try {
    ...
    if (!dryRun) {
      store = options.store ?? createDayPlanStore({
        dbPath: options.dbPath ?? process.env.FORGE_DB_PATH ??
