...
1. **Context collector** (`src/lib/day-plan/brief-sources.ts`) — Cove-owned. Resolves, normalizes, and bounds each source, and records a per-source manifest (`as_of`, hash, freshness, coverage, trims).
   - Required: `~/Atlas/brain/GOALS.md`, the sprint memo (`COVE_BRIEF_SPRINT_MEMO_PATH`), a fresh open-task snapshot via `/api/forge-rest` (default base `http://127.0.0.1:3200`, override `COVE_BRIEF_WEB_BASE`), recent settlement/completion summary.
