...
This is a reversible experiment for learning whether an awake, logged-in Mac can reliably put Cove in front of the user at the right morning seam. It is not wired into Cove's production or client installer.
...
It does not open a plan that is already opened, skipped, bypassed, confirmed, or settled. A failed request, malformed server response, bad receipt file, or unavailable Cove server fails closed. The script never POSTs to Cove and never uses or logs the CSRF token returned by the day-plan endpoint. When no plan exists, opening `/tasks` lets the app perform its normal plan ensure.

One recovery case is deliberate: if the only unsettled plan belongs to an earlier local date, the pulse opens Cove once. The app opens that prior Day Settlement first, then creates today's Morning Arrival only after the previous workday is closed.

## Configure

Create `data/cove-arrival.json` (the entire `data/` directory is already gitignored):
...
The proposed spike-only LaunchAgent is `com.cove.arrival-spike`. Its plist should use:
...
If installed manually, use `~/Library/Logs/cove-arrival-spike.log` for both standard output and error. Lint the temporary plist with `plutil -lint`, atomically move it into `~/Library/LaunchAgents/com.cove.arrival-spike.plist`, then use only:

```bash
launchctl bootout "gui/$(id -u)/com.cove.arrival-spike" 2>/dev/null || true
launchctl bootstrap "gui/$(id -u)" "$HOME/Library/LaunchAgents/com.cove.arrival-spike.plist"
```

This Phase 0 work does not create, install, bootstrap, or modify that plist. It does not touch any existing Cove LaunchAgent or the main installer.

To uninstall a manual experiment, boot out exactly `gui/$(id -u)/com.cove.arrival-spike` and move only its plist out of `~/Library/LaunchAgents`. The delivery receipt can remain for diagnosis or be moved to Trash to reset the experiment.
...
`/usr/bin/open` delegates to the user's default browser. An already-open Cove page may be focused, reused, or accompanied by another tab depending on browser behavior. Duplicate-tab behavior remains an experiment and is not claimed as solved by the receipt policy.
...
| Earlier unsettled workday | One open so Cove can finish Settlement first |
...
```bash
node --test tests/cove-arrival-schedule.test.mjs
```
