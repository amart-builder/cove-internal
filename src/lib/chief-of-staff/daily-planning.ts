...
export function validateDailyDecision(
  ...
  options: { requireNarrative?: boolean } = {},
): DailyDecision {
  ...
  if (watches.some(watch => watch.kind === "calendar" && watch.revision === 0))
...
export function collectPlanningContext(
  ...
  const eventRows = db
    .prepare(freshCalendar
      ...
      : "SELECT id,source_json,observed_at FROM cove_calendar_occurrences WHERE end_at>=? ORDER BY start_at LIMIT 30")
    .all(...(freshCalendar ? [] : [now.toISOString()])) as {
    id: string;
    source_json: string;
    observed_at: string;
  }[];
  ...
  const questions = db
    .prepare(
      "SELECT id,outcome_key,decision_key,question,state,substr(answer,1,600) AS answer,answer_source,next_check_at,revision FROM cove_planning_questions ORDER BY CASE state WHEN 'open' THEN 0 ELSE 1 END,updated_at DESC LIMIT 6",
    )
    .all();
