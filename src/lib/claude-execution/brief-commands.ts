...
export function buildMorningBriefPrompt(input: {
  ...
  return [
    ...
    // The wake loop's planning contract already says this for its own pass
    // ("if coverage is partial, stale, unavailable... retain that
    // uncertainty"). The brief had nothing equivalent, so a morning where Cove
    // cannot see the calendar was free to read exactly like a morning with
    // nothing in it.
