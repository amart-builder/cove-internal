...
type Coverage = {protection?:string;unresolved?:number;enabled:boolean;healthy:boolean;lastCheckedAt:string|null;calendar:{status:string;fresh:boolean};notices:Array<{id:string;title:string;status:string;error?:string;snoozedUntil:string|null}>};
export default function FollowThrough({ compact = false }: { compact?: boolean }) {
 ...
 if(compact) return <a href="/follow-through" className="text-xs text-muted-foreground hover:text-foreground" aria-label="View reminder coverage">{error || !state?.healthy || state.protection === 'attention_required' ? 'Reminder checks need attention' : state.calendar.fresh ? 'Watching deadlines and meetings' : state.calendar.status === 'not_connected' ? 'Watching deadlines · Calendar not connected' : 'Watching deadlines · Calendar needs attention'}</a>;
 return <section aria-label="Reminder coverage" className="mt-5 rounded-xl border border-border bg-card p-5">
  ...
  {state?.enabled && <>
   ...
   {state.protection==='attention_required' && <p role="status" className="mt-2 text-sm text-accent-red">Some reminders need your review. A running checker does not mean every reminder was delivered.</p>}
   {state.notices.filter(n=>['pending','uncertain','failed','missed','delivered'].includes(n.status)).slice(0,5).map(n=><div key={n.id} className="mt-3 flex items-center justify-between gap-3 border-t border-border pt-3 text-sm"><div><p>{n.title}</p><p className="text-xs text-muted-foreground">{n.snoozedUntil && Date.parse(n.snoozedUntil)>Date.now()?'Snoozed for one hour':n.status==='uncertain'?'Delivery was interrupted. Please review this item.':n.status==='missed'?'Meeting reminder was missed. Please review.':n.status==='failed'?'Notification failed. Please review this item.':n.status==='delivered'?'Reminder sent':'Waiting for a suitable reminder time'}</p></div><button type="button" className="shrink-0 text-xs text-accent-blue" onClick={()=>{
   ...
   }}>Snooze 1h</button><button type="button" className="shrink-0 text-xs text-accent-blue" onClick={()=>{
    void getDayPlanCsrfToken().then(token=>fetch('/api/follow-through',{method:'POST',headers:{'Content-Type':'application/json','X-Cove-CSRF':token},body:JSON.stringify({action:'acknowledge',id:n.id})})).then(async response=>{const result=await response.json();if(!response.ok||!result.ok)throw new Error();await refresh();}).catch(()=>setError('Could not acknowledge that reminder. Try again.'));
   }}>On my radar</button></div>)}
  </>}
 </section>;
}
