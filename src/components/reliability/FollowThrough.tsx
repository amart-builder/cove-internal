  ...
  function notice(n: Notice) {
    return <div key={n.id} className="mt-3 border-t border-border pt-3 text-sm">
      ...
      <div className="mt-2 flex flex-wrap gap-4">
        {["pending", "delivered", "uncertain", "failed"].includes(n.status) && <button type="button" disabled={!!busy} className="text-xs text-accent-blue disabled:opacity-50" aria-label={`Snooze 1 hour: ${n.title}`} onClick={() => void update(n, "snooze")}>Snooze 1h</button>}
        <button type="button" disabled={!!busy} className="text-xs text-accent-blue disabled:opacity-50" aria-label={`On my radar: ${n.title}`} onClick={() => void update(n, "acknowledge")}>{busy === n.id ? "Saving..." : "On my radar"}</button>
      </div>
    </div>;
  }
