...
test('Supabase task list and delete requests preserve the pre-stage REST shape', {
  ...
  globalThis.fetch = async (url, init = {}) => {
    ...
    if (String(url).startsWith('/api/forge-rest/tasks') && (init.method ?? 'GET') === 'GET') {
      return new Response('[]');
    }
    if (String(url) === '/api/day-plan') {
      return new Response('{"csrfToken":"mode-token"}');
    }
    if (String(url) === '/api/forge-rest/tasks?id=eq.t1' && init.method === 'DELETE') {
  ...
  const listCall = calls.find((call) => call.url.startsWith('/api/forge-rest/tasks?'));
  assert.equal(listCall.url, '/api/forge-rest/tasks?select=*&order=position.asc');
  const deleteCall = calls.find((call) => call.init.method === 'DELETE');
  assert.equal(deleteCall.url, '/api/forge-rest/tasks?id=eq.t1');
...
test('archive, undo, Recently deleted, and recurrence affordances are local-mode gated', () => {
  ...
  const route = readFileSync(path.join(root, 'src/app/api/forge-rest/[table]/route.ts'), 'utf8');
