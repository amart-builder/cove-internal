...
test("a declined calendar notice keeps the model context after its event line", async (t) => {
  ...
  const handler = createEmailClassificationHandler({
    ...
    gateway: {
      getMessage: async () => ({
        ...
        internalDate: "1000",
        headers: [
          { name: "From", value: "Sam Rivera <sam.rivera@example.com>" },
          {
            name: "Subject",
            value: "Declined: Sam Rivera and Edge AI @ Thu Sep 24, 2026 3pm - 3:30pm (PDT)",
          },
        ],
        snippet: "Sam cannot attend and asked to reschedule.",
        text: "Sam cannot attend and asked to reschedule.",
      }),
      modifyThreadLabels: async () => {},
    },
    classifier: async () => ({
      bucket: "action",
      summary: "Sam cannot attend and asked to reschedule.",
  ...
  assert.deepEqual(
    ...
    {
      bucket: "action",
      summary: "Declined: Sam Rivera and Edge AI (Sam Rivera). Sam cannot attend and asked to reschedule.",
    },
  );
});
