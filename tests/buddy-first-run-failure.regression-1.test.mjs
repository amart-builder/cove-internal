...
import { buddyFailureMessage, isProviderMissing, isProviderNotSignedIn } from '../src/lib/buddy/errors.ts';
...
// The screen is what the person reads, so the codes have to reach it. The
// sign-in card is chosen in the component; every other code is worded by
// buddyFailureMessage, which this line already had for the codes it knew.
test('the screen acts on both codes', () => {
  ...
  assert.match(buddyFailureMessage('provider_missing'), /Cove cannot find the Claude command/);
  assert.match(buddyFailureMessage('provider_missing', 'codex'), /Cove cannot find the Codex command/);
  assert.match(buddyFailureMessage('provider_missing'), /Ask your Cove setup agent to install it/);
});
